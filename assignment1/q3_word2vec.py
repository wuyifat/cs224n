...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    for idx in indices[1:]:
        ...
        grad[idx] -= (sigmoid(np.dot(-uk, vc)) - 1) * vc
    ### END YOUR CODE
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    context_idx = [tokens[w] for w in contextWords]
    context_vec = inputVectors[context_idx]
    predicted = np.sum(context_vec, axis=0)
    target = tokens[currentWord]

    cost, gradIn_pred, gradOut = word2vecCostAndGradient(predicted, target, outputVectors, dataset)
    for idx in context_idx:
        gradIn[idx] += gradIn_pred
    ### END YOUR CODE
