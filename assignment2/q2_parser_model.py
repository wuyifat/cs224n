...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        x = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(x, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        x_init = xavier_weight_init()
        with tf.variable_scope('transform'):
            W = x_init((self.config.embed_size * self.config.n_features, self.config.hidden_size))
            b1 = tf.Variable(tf.zeros([self.config.hidden_size,]))
            U = x_init((self.config.hidden_size, self.config.n_classes))
