...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        # s = sentence + [zero_vector]*max_length
        # l = labels + [zero_label]*max_length
        # s = s[:max_length]
        # l = l[:max_length]
        # mask = [ll != zero_label for ll in l]
        # ret.append((s,l,mask))
        ...
        if len(labels) >= max_length:
            ...
            padlen = max_length - len(labels)
            s = sentence + [zero_vector] * padlen
            l = labels + [zero_label] * padlen
            mask = [True] * len(labels) + [False] * padlen
        ret.append((s,l,mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(features, [-1, self.max_length, Config.n_features * Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.Variable(tf.zeros(Config.n_classes))
        h_t = tf.Variable(tf.zeros((1, Config.hidden_size)))
        ### END YOUR CODE
